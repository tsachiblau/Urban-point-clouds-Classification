...
elseif inputFlag==2
    ...
    cloudData = load('bildstein_station5_xyz_intensity_rgb.txt');
    cloudData = cloudData(:,1:3);
    realLables = load('bildstein_station5_xyz_intensity_rgb.labels');
...
addpath(genpath('.'));
visualize = 0;
pixelSize = 0.5; % [m], side length of height-map pixels
blockSideLength = 5.6; % [m], max building-side-length
elevationAngleThresh = 35*pi/180; % [rad], max ground slope angle
maxPointHeightDiff=0.5;  % [m]
