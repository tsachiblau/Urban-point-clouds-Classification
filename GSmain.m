...
clear all;
addpath(genpath('.'));
inputFlag = 1;    % paris = 0 , GS = 1 , S3D=2
...
elseif inputFlag==1
    ...
    AirOrigCloudFile = 'RA11-12_airborneGT.mat';
    terrestrialOrigCloudFile= 'RA11-12_terrestrialGT.mat';
    ...
    realLablesTerrestrial = load('RA11-12_terrestrialGT');
    realLablesTerrestrial = realLablesTerrestrial.pointClass_terrestrial;
    realLablesairborne = load('RA11-12_airborneGT');
...
elseif inputFlag==2
    ...
    cloudData = load('domfountain_station2_xyz_intensity_rgb.txt');
    cloudData = cloudData(:,1:3);
    realLables = load('domfountain_station2_xyz_intensity_rgb.labels');
...
blockSideLength = 40; % [m], max building-side-length
elevationAngleThresh = 25*pi/180; % [rad], max ground slope angle
maxPointHeightDiff=0.20;  % [m]
...
%
    ...
    curvatureThreshold = 1/10;
    minClusterSize = floor(A/(voxleSize^2));
    maxDistance=2; %min distance between points that we want to add to region
    tic
