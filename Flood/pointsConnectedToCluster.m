...
%% calc flowTH
    ...
    times=min(numberOftimes,floor(lengthOfSource/2));
    seeds=floor(linspace(1,lengthOfSource,times*2));
    mfVec=zeros(times,1);
    for i=1:2:times
      	[mfVec(ceil(i/2)),~,~,~]=maxflow(T,seeds(i),seeds(i+1));  
    end
    minWightPerWall=mean(mfVec);
%     figure;
%     hist(mfVec);
    bigEnough=mfVec>minWightPerWall;
%   sum(bigEnough)
    flowTH=mean(mfVec(bigEnough))-sqrt(var(mfVec)/2);
%% calc curvetureTH
...
%         highlight(p1,4386,'NodeColor','green','MarkerSize',10);
            ...
            if length(ct)>=1 & length(cs)>=1
                ...
                while mf<flowTH & allNan==false & length(cs)>=1 & length(ct)>=1
                    ...
                    if rmLabledIdx==0
                        ...
                        if length(goodIdx)>0
                            [mf,~,cs,ct]=maxflow(T,topListSourcesNeedToCheckIdx(goodIdx),sink(1));
                            mf=mf+getWeightToPoint(xyz(sink(1),:),flowTH,minZofCluster,curvature(sink(1)),curvetureTH,mf);
%                             close all;
%                             figure;
%                             p1=plot(T,'XData',xyz(1:size(T.Nodes,1),1),'YData',xyz(1:size(T.Nodes,1),2),'ZData',xyz(1:size(T.Nodes,1),3));
